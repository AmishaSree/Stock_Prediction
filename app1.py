...
def predict_stock(df):
    ...
    # --- Previous 10 Days Prediction ---
    prev_forecast = results.get_prediction(start=len(data)-10, end=len(data)-1)
    prev_pred = prev_forecast.predicted_mean
    prev_dates = pd.date_range(end=current_date - timedelta(days=1), periods=len(prev_pred))

    prev_df = pd.DataFrame({
        'Date': prev_dates,
        'Predicted Close': prev_pred.values
    })

    # --- Next 10 Days Prediction ---
    next_forecast = results.get_forecast(steps=10)
    next_pred = next_forecast.predicted_mean
    next_dates = pd.date_range(start=current_date, periods=len(next_pred))

    next_df = pd.DataFrame({
        'Date': next_dates,
        'Predicted Close': next_pred.values
    })

    return prev_df, next_df, data

# --- STREAMLIT UI ---
st.title("📈 Time-Series Analysis of Stock Prices Using SARIMA")
...
if not companies:
    ...
    if st.button("🔮 Predict Stock Prices"):
        with st.spinner('Training model and generating predictions...'):
            ...
            ax.plot(prev_df['Date'], prev_df['Predicted Close'], label='Prev 10 Days', color='blue')
            ax.plot(next_df['Date'], next_df['Predicted Close'], label='Next 10 Days', color='red')
